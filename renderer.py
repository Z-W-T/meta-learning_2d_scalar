    ...
    def render(self, off_screen=True, file_name=None, view_blocks=2):
        ...
        for i, path_ in enumerate(self.path):
            if path_ is None:
                ...
                reader.SetDimensions(self.array[i].shape[1], self.array[i].shape[0], 2)
                vtk_data = vtkmodules.util.numpy_support.numpy_to_vtk(np.stack((self.array[i], self.array[i]), axis=0).ravel(), deep=True, array_type=vtk.VTK_FLOAT)
        ...
        opacityTransferFunction.AddPoint(1, 0.3)
        ...
        colorTransferFunction.AddRGBPoint(0, 0.231, 0.298, 0.752)
        colorTransferFunction.AddRGBPoint(0.5, 0.865, 0.865, 0.865)
        colorTransferFunction.AddRGBPoint(1, 0.705, 0.015, 0.149)

        # 创建并设置标量条 (vtkScalarBarActor)
        ...
        renWin.SetSize(2000*view_blocks, 1000*view_h)
        ...
        if off_screen:
            ...
            iren.Initialize()
            try:
                iren.CreateRepeatingTimer(5000)
                def close_window(obj, event):
                    # 停止定时器
                    obj.DestroyTimer()
                    # # 安全关闭窗口
                    # if obj.GetRenderWindow() is not None:
                    #     obj.GetRenderWindow().Finalize()
                    obj.TerminateApp()
                # iren.AddObserver("TimerEvent", close_window)
                renWin.Render()
                iren.Start()
